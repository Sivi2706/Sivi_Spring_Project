...
TURN_THRESHOLD = 80       # Threshold for line positioning

# Recovery parameters
REVERSE_DURATION = 0.5    # seconds to reverse
REVERSE_SPEED = 30        # speed when reversing
SCAN_ANGLES = [-90, -45, 0, 45, 90]  # Scanning angles
...
def map_custom_angle_to_servo(custom_angle):
    """
    Map custom angle range (-90 to 90) to servo range (0 to 180)
    -90 degrees -> 0 servo degrees (right most)
    0 degrees -> 90 servo degrees (center)
    90 degrees -> 180 servo degrees (left most)
    """
    # Clamp the input angle to the -90 to 90 range
    clamped_angle = max(-90, min(90, custom_angle))
    
    # Linear mapping from -90:90 to 0:180
    servo_angle = ((clamped_angle + 90) / 180) * 180
...
def set_servo_angle(servo_pwm, custom_angle):
    # Map custom angle to servo angle
    servo_angle = map_custom_angle_to_servo(custom_angle)
    
    # Ensure angle is within 0-180 range
    servo_angle = max(0, min(180, servo_angle))
    
    duty = SERVO_MIN_DUTY + (servo_angle * (SERVO_MAX_DUTY - SERVO_MIN_DUTY) / 180.0)
...
# Line detection function with anchoring
def detect_line_with_anchor(frame):
    ...
    if contours:
        ...
        if area > MIN_CONTOUR_AREA:
            ...
            if M["m00"] != 0:
                ...
                # Draw the centroid (blue dot anchor)
...
def main():
    ...
    try:
        while True:
            ...
            error, line_found, annotated_frame = detect_line_with_anchor(frame)
            
            # Display the frame
            cv2.imshow("Line Follower", annotated_frame)
            ...
            if state == "NORMAL":
                if line_found:
                    ...
                    if error > TURN_THRESHOLD:
                        ...
                        set_servo_angle(servo_pwm, -45)  # Adjust servo angle during turn
                        ...
                        set_servo_angle(servo_pwm, 0)    # Center servo
                ...
                if time.time() - scan_start_time >= SCAN_TIME_PER_ANGLE:
                    ...
                    frame = picam2.capture_array()
                    error, line_found, _ = detect_line_with_anchor(frame)
                    if line_found:
