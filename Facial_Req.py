...
class SymbolRecognizer:
    def __init__(self, symbol_dir):
        ...
        self.symbol_templates = {}  # Dictionary: symbol_name -> (color_template, gray_template)
        self.calibrate()
            ...
            if os.path.isdir(subfolder_path):
                for filename in os.listdir(subfolder_path):
                    if filename.lower().endswith('.png'):
                        ...
                        # Load template in BGR format (default for OpenCV)
                        template_color = cv2.imread(full_path, cv2.IMREAD_COLOR)
                        
                        if template_color is not None:
                            # Ensure BGR format consistency
                            template_color = cv2.cvtColor(template_color, cv2.COLOR_RGB2BGR)

                            # Convert to grayscale for matching
                            template_gray = cv2.cvtColor(template_color, cv2.COLOR_BGR2GRAY)

                            # Resize template to a fixed size (100x100) for consistency
                            template_color = cv2.resize(template_color, (100, 100))
                            template_gray = cv2.resize(template_gray, (100, 100))

                            symbol_name = f"{subfolder}_{os.path.splitext(filename)[0]}"
                            self.symbol_templates[symbol_name] = (template_color, template_gray)

                            print(f"Loaded template: {symbol_name}, Shape: {template_color.shape}")
    ...
    def match_symbol(self, roi_gray):
        ...
        for name, (_, template_gray) in self.symbol_templates.items():
            try:
                print(f"Matching {name} with ROI size {roi_gray.shape}")
